...
class UserProfile(models.Model):
    ...
    def trust_score(self):
        ...
        late_payments = MoneyTransaction.objects.filter(borrower=self.user, status='DEFAULTED').count()
        score -= (late_payments * 20)
        
        # Reward for successful repayments
        successful_repayments = MoneyTransaction.objects.filter(borrower=self.user, status='REPAID').count()
    ...
    def total_lends(self):
        ...
        goods_lends = self.user.goods_lend_transactions.exclude(status='REJECTED').count()
...
class LoanOffer(models.Model):
    ...
    interest_rate = models.DecimalField(max_digits=5, decimal_places=2) # Monthly interest
...
class MoneyTransaction(models.Model):
    ...
    lender = models.ForeignKey(User, on_delete=models.CASCADE, related_name='money_lends')
    borrower = models.ForeignKey(User, on_delete=models.CASCADE, related_name='money_borrows')
    loan_offer = models.ForeignKey(LoanOffer, on_delete=models.SET_NULL, null=True, blank=True, related_name='transactions')
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    ...
    payment_status = models.CharField(max_length=20, default='NOT_PAID') # NOT_PAID, INITIATED, COMPLETED, FAILED
...
class GoodsExchange(models.Model):
    ...
    owner = models.ForeignKey(User, on_delete=models.CASCADE, related_name='goods_offered')
...
class GoodsTransaction(models.Model):
    ...
    item = models.ForeignKey(GoodsExchange, on_delete=models.CASCADE, related_name='transactions')
    lender = models.ForeignKey(User, on_delete=models.CASCADE, related_name='goods_lend_transactions')
    borrower = models.ForeignKey(User, on_delete=models.CASCADE, related_name='goods_borrow_transactions')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='PENDING')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Review(models.Model):
    ...
    money_transaction = models.ForeignKey(MoneyTransaction, on_delete=models.SET_NULL, null=True, blank=True, related_name='reviews')
    goods_transaction = models.ForeignKey(GoodsTransaction, on_delete=models.SET_NULL, null=True, blank=True, related_name='reviews')
...
class FineReward(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='fines_rewards')
    amount = models.DecimalField(max_digits=10, decimal_places=2)
