...
from .models import UserProfile, MoneyTransaction, GoodsExchange, GoodsTransaction, Friendship, FriendRequest, ChatMessage, Notification, FineReward, PasswordResetCode, LoanOffer, Review, SignupOTP
...
from .forms import KYCForm, LoanRequestForm, GoodsListingForm
import logging
import uuid
import json
import base64
...
def signup_verify_otp(request):
    ...
    if request.method == 'POST':
        ...
        otp_record = SignupOTP.objects.filter(email=email, code=code).order_by('-created_at').first()
        ...
        if otp_record and otp_record.is_valid():
            ...
            try:
                ...
                if referrer_code:
                    referrer_profile = UserProfile.objects.filter(referral_code=referrer_code).first()
                ...
                if referrer_profile:
                    ...
                    profile.save()
                    messages.success(request, f'Referral bonus applied! You got 50 points and {referrer_profile.user.username} got 100 points.', extra_tags='auth')

                auth_login(request, user)
                messages.success(request, 'Account verified and created successfully! Please complete KYC verification.', extra_tags='auth')
                ...
                messages.error(request, f'Error creating account: {str(e)}', extra_tags='auth')
        else:
...
def kyc_submit(request):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, 'KYC submitted successfully! It will be reviewed within 24 hours.', extra_tags='kyc')
        return redirect('user_dashboard')
...
def user_dashboard(request):
    ...
    # Financial Stats
    active_borrows = MoneyTransaction.objects.filter(borrower=request.user, status='ACTIVE')
    active_lendings = MoneyTransaction.objects.filter(lender=request.user, status='ACTIVE')
    ...
    usage_percent = (active_borrows_amount / borrow_limit * 100) if borrow_limit > 0 else 0
    ...
    # Social Stats
    friends_count = Friendship.objects.filter(Q(user1=request.user) | Q(user2=request.user)).count()
    
    # Notifications
    notifications = Notification.objects.filter(user=request.user).order_by('-created_at')[:5]
    
    # Pending Lend Requests (Needs Approval/Funding)
    pending_lend_requests = MoneyTransaction.objects.filter(lender=request.user, status='PENDING')
...
def marketplace(request):
    ...
    goods_items = GoodsExchange.objects.filter(available=True).exclude(owner=request.user)
    ...
    borrow_requests = MoneyTransaction.objects.filter(status='PENDING').exclude(borrower=request.user)
...
def borrow(request, offer_id=None):
    ...
    if request.method == 'POST':
        ...
        if active_borrows.exists():
            messages.error(request, "You already have an active borrowing. Pay it back to borrow again.")
            return redirect('transactions')
            
        if amount > profile.borrowing_limit:
            messages.error(request, f"Amount exceeds your borrowing limit of {profile.borrowing_limit}")
        ...
        # Notify lender
        Notification.objects.create(user=lender, message=f"{request.user.username} has requested the loan offer of Rs. {amount} from you.")
        
        messages.success(request, "Loan request sent successfully! Lender needs to approve it.")
        return redirect('transactions')

    # If GET
    if offer:
        return render(request, 'user/borrow_confirm.html', {'offer': offer, 'profile': profile})
    
    potential_lenders = UserProfile.objects.filter(kyc_verified=True).exclude(user=request.user)
    return render(request, 'user/borrow.html', {'lenders': potential_lenders, 'profile': profile})
...
def lend(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'post_offer':
            ...
            if amount > profile.lending_limit:
                messages.error(request, f"Amount exceeds your lending limit of {profile.lending_limit}")
                return redirect('lend')
            ...
            transaction = get_object_or_404(MoneyTransaction, id=request_id, lender=request.user, status='PENDING')
    ...
    pending_requests = MoneyTransaction.objects.filter(lender=request.user, status='PENDING')
...
def goods(request):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f"Item listed for {exchange_type.lower()} successfully!")
        return redirect('goods')

    # Filter available goods from others
    available_goods = GoodsExchange.objects.filter(available=True).exclude(owner=request.user)
...
def request_exchange(request, item_id):
    ...
    existing = GoodsTransaction.objects.filter(item=item, borrower=request.user, status='PENDING').exists()
    if existing:
...
def manage_exchange(request, transaction_id):
    ...
    transaction = get_object_or_404(GoodsTransaction, id=transaction_id, lender=request.user)
...
def complete_exchange(request, transaction_id):
    ...
    if action == 'complete': # Borrower confirms receipt
        if transaction.borrower == request.user and transaction.status == 'APPROVED':
            ...
            transaction.save()
            messages.success(request, "Exchange marked as completed! Please rate the user.")
        else:
...
def my_exchanges(request):
    ...
    my_requests = GoodsTransaction.objects.filter(borrower=request.user).order_by('-created_at')
    incoming_requests = GoodsTransaction.objects.filter(lender=request.user).order_by('-created_at')
...
def friends(request):
    ...
    if query:
        ...
        ).exclude(id=request.user.id).exclude(is_superuser=True)[:10]
    ...
    if request.method == 'POST':
        ...
        if action == 'add':
            ...
            fr = get_object_or_404(FriendRequest, from_user=target_user, to_user=request.user)
            fr.accepted = True
            fr.save()
            ...
            messages.success(request, f"Friend request from {target_username} rejected.")
        elif action == 'remove':
...
def submit_review(request):
    ...
    if request.method == 'POST':
        ...
        points_awarded = 0
        r_val = int(rating)
        if r_val == 5: points_awarded = 20
        elif r_val == 4: points_awarded = 10
        elif r_val == 3: points_awarded = 5
        ...
        messages.success(request, f"Review for {reviewee.username} submitted! {points_awarded} reward points added to their profile.")
...
def reviews(request):
    ...
    received_reviews = Review.objects.filter(reviewee=target_user).order_by('-timestamp')
...
def chat(request):
    ...
    sent_ids = ChatMessage.objects.filter(sender=request.user).values_list('receiver', flat=True)
    received_ids = ChatMessage.objects.filter(receiver=request.user).values_list('sender', flat=True)
    ...
    for partner in chat_partners:
        ...
        unread_count = ChatMessage.objects.filter(sender=partner, receiver=request.user, is_read=False).count()
    ...
    conversations.sort(key=lambda x: x['last_message']['timestamp'] or aware_min, reverse=True)
    ...
    if active_user_id:
        ...
        if request.method == 'POST':
            ...
            if content:
                ChatMessage.objects.create(sender=request.user, receiver=active_chat_user, message=content)
                return redirect(f"{reverse('chat')}?user_id={active_user_id}")
        
        # Mark messages as read
        ChatMessage.objects.filter(sender=active_chat_user, receiver=request.user, is_read=False).update(is_read=True)
        ...
        if not is_friend:
            status_alert = "Do you know this person? Be cautious when dealing with strangers."
...
def get_messages(request, user_id):
    ...
    query = Q(sender=other_user, receiver=request.user) | Q(sender=request.user, receiver=other_user)
    if last_id:
        messages = ChatMessage.objects.filter(query, id__gt=last_id).order_by('timestamp')
    else:
...
def transactions(request):
    ...
    borrows = MoneyTransaction.objects.filter(borrower=request.user).order_by('-created_at')
    lends = MoneyTransaction.objects.filter(lender=request.user).order_by('-created_at')
    goods_borrows = GoodsTransaction.objects.filter(borrower=request.user).order_by('-created_at')
    goods_lends = GoodsTransaction.objects.filter(lender=request.user).order_by('-created_at')
    ...
    if search_q:
        borrows = borrows.filter(Q(lender__username__icontains=search_q) | Q(lender__first_name__icontains=search_q) | Q(lender__last_name__icontains=search_q))
        lends = lends.filter(Q(borrower__username__icontains=search_q) | Q(borrower__first_name__icontains=search_q) | Q(borrower__last_name__icontains=search_q))
        goods_borrows = goods_borrows.filter(Q(item__item_name__icontains=search_q) | Q(lender__username__icontains=search_q) | Q(lender__first_name__icontains=search_q) | Q(lender__last_name__icontains=search_q))
        goods_lends = goods_lends.filter(Q(item__item_name__icontains=search_q) | Q(borrower__username__icontains=search_q) | Q(borrower__first_name__icontains=search_q) | Q(borrower__last_name__icontains=search_q))
...
def admin_dashboard(request):
    ...
    context = {
        ...
        'new_users_today': User.objects.filter(date_joined__date=today).exclude(is_superuser=True).count(),
        'active_loans': MoneyTransaction.objects.filter(status='ACTIVE').count(),
        'total_volume': MoneyTransaction.objects.aggregate(total=Avg('amount'))['total'] or 0, # Should be sum, fixing below
        'recent_transactions': MoneyTransaction.objects.all().order_by('-created_at')[:5],
        'recent_users': User.objects.exclude(is_superuser=True).order_by('-date_joined')[:5],
    }
    
    # Correction: total_volume should be Sum
    from django.db.models import Sum
    context['total_volume'] = MoneyTransaction.objects.aggregate(total=Sum('amount'))['total'] or 0
...
def admin_user_management(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'delete':
            ...
            messages.success(request, f"User {target_user.username} {status}.", extra_tags='admin')
...
def admin_kyc_verification(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'approve':
            ...
            profile.save()
            Notification.objects.create(user=profile.user, message="Your KYC has been approved! You can now start lending and borrowing.")
            messages.success(request, f"Approved KYC for {profile.user.username}", extra_tags='admin')
        elif action == 'reject':
            profile.kyc_status = 'REJECTED'
            profile.kyc_verified = False
            profile.save()
            Notification.objects.create(user=profile.user, message="Your KYC was rejected. Please resubmit with correct details.")
            messages.error(request, f"Rejected KYC for {profile.user.username}", extra_tags='admin')
            
        return redirect('admin_kyc_verification')

    return render(request, 'admin/kyc_verification.html', {'kyc_requests': kyc_requests})
...
def admin_reports(request):
    ...
    all_time_stats = {
        ...
        'total_amount': MoneyTransaction.objects.aggregate(total=Sum('amount'))['total'] or 0,
...
def admin_user_detail(request, user_id):
    ...
    active_borrows = MoneyTransaction.objects.filter(borrower=target_user, status='ACTIVE')
    active_lendings = MoneyTransaction.objects.filter(lender=target_user, status='ACTIVE')
...
def esewa_initiate(request, transaction_id):
    ...
    transaction = get_object_or_404(MoneyTransaction, id=transaction_id, lender=request.user)
    ...
    return render(request, 'user/esewa_redirect.html', {
        'payload': payload,
...
def esewa_repayment_initiate(request, transaction_id):
    ...
    transaction = get_object_or_404(MoneyTransaction, id=transaction_id, borrower=request.user)
...
def esewa_callback(request):
    ...
    if data.get('status') == 'COMPLETE':
        if transaction.status == 'PENDING':
            ...
            messages.success(request, f"Payment successful! Loan to {transaction.borrower.username} is now active.")
            ...
            messages.success(request, f"Repayment successful! Loan from {transaction.lender.username} is now repaid.")
    else:
        transaction.payment_status = 'FAILED'
        transaction.save()
        messages.error(request, f"Payment failed or was cancelled. Status: {data.get('status')}")
...
def view_public_profile(request, user_id):
    ...
    # Statistics
    stats = {
        'successful_loans': MoneyTransaction.objects.filter(lender=target_user, status='REPAID').count(),
        'active_loans': MoneyTransaction.objects.filter(lender=target_user, status='ACTIVE').count(),
